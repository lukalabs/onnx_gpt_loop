...
def export_one_step_model(model: OneStepTorchModel, out_file_path):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        input_names = ['input_ids', 'position_ids', 'attention_mask', 'temperature', 'top_k']
        input_names += input_past_key_values_names
        output_names = ['next_input_ids', 'next_position_ids', 'next_attention_mask']
        output_names += output_past_key_values_names
        torch.onnx.export(
            model=model,
            args=tuple([input_ids, position_ids, attention_mask, temperature, top_k] + past_key_values),
            f=onnx_file_path,
...
def _extract_loop_body_and_graph_inputs(file_path):
    ...
    position_ids = loop_body.input[1]
    attention_mask = loop_body.input[2]
    past_key_values = loop_body.input[3:]
    # ['input_ids', 'position_ids', 'attention_mask', *'past_key_values_{i}']
    ...
    # ['i_step', 'cond', 'input_ids', 'position_ids', 'attention_mask', *'past_key_values_{i}']

    next_input_ids = loop_body.output[0]
    loop_body.output.insert(0, cond)
    loop_body.output.append(next_input_ids)
    # ['cond', 'next_input_ids', 'next_position_ids', 'attention_mask', *'output_past_key_values_{i}', next_input_ids]

    graph_inputs = [temperature, top_k, input_ids, position_ids, attention_mask, *past_key_values]
...
def _make_graph(loop_node, graph_inputs):
    ...
    graph = helper.make_graph(
        ...
        name='graph',
        # ['n_steps', 'input_ids', position_ids, attention_mask, 'temperature', 'top_k', *'past_key_values_{i}']:
